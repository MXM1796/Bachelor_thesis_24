...
It looks as if the package \Package{tabularray} can be used for all tweaking of tables.
It includes options related to \Package{amsmath}, \Package{booktabs}, \Package{longtable},
\Package{siunitx} and more.
There are improved ways of spanning columns and rows,
removing the need for \Macro{multicolumn} and the \Package{multirow} package.
There are many options for also making colourful tables, adjusting spacing etc.,
while the defautl row spacing looks to be better than the standard one in the \Env{tabular} and \Env{array} environments.
The package was only released in 2021, so you need a recent version of \TeXLive to be able to use it.

Tables usually go inside the \Env{table} environment so their position can \enquote{float}.
In this chapter I use some tables inside \Env{table} and some inline.
...
Here are very similar tables to those in the previous section
typeset using the \Package{tabularray} package
and the \Env{tblr} and \Env{booktabs} environments, respectively.
The column widths are fixed to show the effect of the \Option{table-number-alignment} option.
The \Option{vlines} option is used to show the effect of the alignment.
Note that triple pairs of braces have to be used in these environments
to guard non-numeric cells, such as column headings,
if one is using the \Option{S} column descriptor or 
the \Option{Q} column descriptor with the \Option{si} key.
In the latter case, the \Option{c} option applies
...
\end{center}
  ...
  \missingfigure{TeX Live version does not include \Package{tabularray}}
}

A further simple example is shown in \cref{tab:viscosity}.
...
\begin{tabular}{lS[table-format=4.2]S}
  ...
  &  {[\unit{\celsius}]} & {(\unit{\metre\pascal\second})} \\
  \midrule
  Blood     & 37 & 4.0 \\
  Glycerine &  0 & 10000 \\
  & 20 & 1410 \\
...
\Cref{tab:xsect1b} is rotated by \qty{90}{\degree} to illustrate how that can be done.
